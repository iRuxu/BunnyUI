...
gulp.task("template", function() {
    ...
    let htmlfiles = gulp.src([
        __path(SRC, CONF.source.html, "**/*.html"),
        ...ignore_files
    ]);
        ...
        if (scheme_status == "development") {
            ...
            let custom_diff = {
                output: {
                    path: path.resolve(
                        __dirname,
                        CONF.build[scheme_status].path.js
                    )
                }
            };
            let custom_config = lodash.merge(
                {},
                webpackConfig[1],
...
gulp.task("build", function() {
    ...
    if (!CONF.build[scheme_status]) {
        console.error(
            "\u00d7\0\0error : 指定的build方案不存在，请检查kaci.config.js中build配置项"
        );
        return;
    }
    let current_scheme = lodash.merge(
        {},
        DEVELOPMENT,
        CONF.build[scheme_status]
    );
    ...
    console.log(
        "\u2714\0\0success : build 完成 =>",
        __path(__dirname, SCHEME.path.root)
    );
});
